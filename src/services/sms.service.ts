...
export default class SmsService {
  ...
  public verify = async (phoneNumber:string) => {
    const mapper = {
        [SMSGateways.TWILIO]: this.initiateTwilioPhoneNumberVerification(phoneNumber),
    }
  ...
  public confirm = async (phoneNumber:string, verificationCode:string) => {
    const mapper = {
        [SMSGateways.TWILIO]: this.confirmTwilioPhoneNumber(phoneNumber, verificationCode),
    }
    return mapper[smsGateway]; 
  }
  ...
  public sendMessage = async (receiverPhoneNumber:string, message:string) => {
    const mapper = {
        [SMSGateways.TWILIO]: this.sendTwilioMessage(receiverPhoneNumber, message),
        [SMSGateways.TERMII]: this.sendTermiiMessage(receiverPhoneNumber, message),
    }
    return mapper[smsGateway];
  }

  //Twilio
  private initiateTwilioPhoneNumberVerification = async(phoneNumber: string) => {
    return await this.twilioClient.verify.services(verificationServiceSID)
      .verifications
      .create({ to: phoneNumber, channel: 'sms' })
  }

  private confirmTwilioPhoneNumber = async(phoneNumber: string, verificationCode: string) => {
    const result = await this.twilioClient.verify.services(verificationServiceSID)
      .verificationChecks
...
private sendTwilioMessage = async (receiverPhoneNumber: string, message: string) => { 
